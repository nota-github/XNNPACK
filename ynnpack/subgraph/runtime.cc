...
// 1) infer symbolic backward extents for all buffers to ensure loops are only
//    fused if they share a common backward extent origin.
...
//    or backward extents inferred in 1).
...
void ynn_runtime::schedule() {
  ...
  // This pass infers symbolic backward extents for all buffers, traversing the
  // pipeline in reverse topological order (consumers to producers). It ensures
  // that loops are only fused if their dimensions share a common backward
  // extent origin, which naturally prevents incorrect fusions of unrelated
  // dimensions that happen to have the same constant size. This is simimlar
  ...
  // Maps {buffer_sym, dim_index} to its inferred backward extent expression.
  std::map<std::pair<slinky::var, int>, slinky::expr> backward_extents;

  // Creates a new default symbolic backward extent variable.
  auto get_default_backward_extent = [&](slinky::var buf, int dim) {
    return slinky::variable::make(globals.symbols.insert(
        "backward_" + globals.symbols.name(buf) + "_" + std::to_string(dim)));
  };

  // Lazily creates a new symbolic backward extent variable if one doesn't
  // exist.
  auto get_backward_extent = [&](slinky::var buf, int dim) {
    auto key = std::make_pair(buf, dim);
    if (backward_extents.find(key) == backward_extents.end()) {
      backward_extents[key] = get_default_backward_extent(buf, dim);
    }
    return backward_extents[key];
  };

  // Traverses operations backwards to propagate backward extent symbols.
  for (int i = funcs.size() - 1; i >= 0; --i) {
    ...
    for (const auto& in : f.inputs()) {
      for (int d = 0; d < in.bounds.size(); ++d) {
        ...
        std::vector<slinky::var> correlated_vars;
        for (auto v : out_vars) {
          if (slinky::depends_on(bound, v).any()) {
            correlated_vars.push_back(v);
          }
        }

        slinky::expr inferred_extent = get_default_backward_extent(in.sym(), d);

        if (correlated_vars.size() == 1) {
          slinky::var v = correlated_vars[0];

          // Collect backward extents for this variable from all outputs that
          // contain it.
          std::vector<slinky::expr> parent_backward_extents;
          for (const auto& out : f.outputs()) {
            for (int od = 0; od < out.dims.size(); ++od) {
              if (out.dims[od] == v) {
                parent_backward_extents.push_back(
                    get_backward_extent(out.sym(), od));
              }
            }
          }

          if (prove_true(bound.min == v) && prove_true(bound.max == v) &&
              !parent_backward_extents.empty()) {
            // Check if all parent extents are equivalent.
            bool all_equal = true;
            for (size_t k = 1; k < parent_backward_extents.size(); ++k) {
              if (!prove_true(parent_backward_extents[0] ==
                              parent_backward_extents[k])) {
                all_equal = false;
                break;
              }
            }
            if (all_equal) {
              inferred_extent = parent_backward_extents[0];
            }
          }
        }

        auto key = std::make_pair(in.sym(), d);
        if (backward_extents.count(key) > 0) {
          ...
          // dimension).
          if (!prove_true(backward_extents[key] == inferred_extent)) {
            backward_extents[key] =
                slinky::variable::make(globals.symbols.insert(
                    "backward_" + globals.symbols.name(in.sym()) + "_merged_" +
                    std::to_string(d)));
          }
        } else {
          backward_extents[key] = inferred_extent;
        }
      }
    }
  }
  ...
  for (int i = funcs.size() - 1; i >= 0; --i) {
    ...
    if (sched && !sched->loop_splits.empty()) {
      ...
      for (int split_i = 0; split_i < loop_splits.size(); ++split_i) {
        ...
        // exact same inferred backward extent symbol.
        bool extents_match = false;
        if (producer_dim != -1 && consumer_dim != -1 &&
            producer_buf.defined() && consumer_buf.defined()) {
          slinky::expr producer_backward_extent =
              get_backward_extent(producer_buf, producer_dim);
          slinky::expr consumer_backward_extent =
              get_backward_extent(consumer_buf, consumer_dim);

          if (prove_true(producer_backward_extent ==
                         consumer_backward_extent)) {
            extents_match = true;
          }
        }
